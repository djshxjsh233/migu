...
const SCAN_N = 200;           // 扫描多少个IP(调大更准但更慢)
const FS = require('fs');
...
function genCandidates() {
  ...
  const HOT = [
    "104.16.0.0/13","104.24.0.0/14","172.64.0.0/13","162.158.0.0/15",
    "162.159.0.0/16","198.41.128.0/17","173.245.48.0/20"
  ];
  let guard = 0;
  while (cands.size < SCAN_N && guard < SCAN_N * 200) {
    guard++;
    const seg = HOT[myRnd() % HOT.length];
    const [base, bits] = seg.split('/');
    const bp = base.split('.').map(Number);
...
const GITEE_TOKEN = "你的GITEE_TOKEN这里填"; // 你的Gitee token
...
// ============ 订阅推送配置(GitHub双保险) ============
const GH_TOKEN = "你的GITHUB_TOKEN在这里填"; // GitHub token
...
async function main() {
  ...
  const toVerify = tcpAlive.slice(0, Math.min(40, tcpAlive.length));
  console.log(`[优选] TCP可用 ${tcpAlive.length} 个, 对 ${toVerify.length} 个做TLS+SNI+HTTP验证...`);
  const verifiedOk = [];
